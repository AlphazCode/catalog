...
export default function WishlistUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...wishlistRecord };
    setUser_id(cleanValues.user_id);
    setPrice(cleanValues.price);
    setProducts(cleanValues.Products);
    setWishlistProductsId(cleanValues.wishlistProductsId);
    setErrors({});
  };
  const [wishlistRecord, setWishlistRecord] = React.useState(wishlist);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(Wishlist, id) : wishlist;
  ...
  return (
    ...
    >
      ...
      <TextField
        ...
        isRequired={false}
        isReadOnly={false}
        defaultValue={wishlistProductsId}
        onChange={(e) => {
          let { value } = e.target;
          if (onChange) {
            const modelFields = {
